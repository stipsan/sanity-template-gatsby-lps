          ...
          <div className="flex flex-wrap justify-center mx-auto">
            {
                services.map((service, index) => {
                    return <div className="flex-grow-0 flex-shrink-0 flex justify-center items-center flex-col capitalize py-3 px-4 m-3 phablet:m-4 bg-white rounded-md phablet:px-11 phablet:py-4 retina:py-8 retina:px:12" key={index}>
                        <div><img className="mb-4" src={service.icon.asset.url} alt={`${service.label} Icon`} /></div>
                        <div className="w-24 leading-7 tracking-tighter font-semibold text-[22px] tablet:w-36 retina:w-52">{service.label}</div>
                    </div>
                })
            }
          </div>
